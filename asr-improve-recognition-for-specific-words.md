...
## Overview of Riva NIM customization techniques

The following flow diagram shows the Riva ASR NIM along with the possible customizations.
...
| 4. Retrain language model               | Moderate        | Train a new language model for the application domain to improve the recognition of domain specific terms.     | When domain text data is available.                        | [Tutorial](https://github.com/nvidia-riva/tutorials/blob/stable/asr-python-advanced-nemo-ngram-training-and-finetuning.ipynb) |
| 5. Fine-tune an existing acoustic model | Moderately hard | Fine-tune an existing acoustic model using a small amount of domain data to better suit the domain. | When transcribed domain audio data is available (10h-100h), and other easier approaches fall short.   | [Conformer-CTC - Fine-Tuning Tutorial](https://github.com/nvidia-riva/tutorials/blob/main/asr-finetune-conformer-ctc-nemo.ipynb), [ZH_Conformer-CTC - Fine-Tuning Tutorial](https://github.com/nvidia-riva/tutorials/blob/main/Language-Scaling/Mandarin/asr-finetune-conformer-ctc-nemo-zh.ipynb), [Conformer-CTC - Deployment Tutorial](https://github.com/nvidia-riva/tutorials/blob/main/asr-deploy-conformer-ctc.ipynb) |
...
Out of the box vocabulary files for Riva languages can be found on NGC, for example, for English, the vocabulary file named `flashlight_decoder_vocab.txt` can be found at this [link](https://catalog.ngc.nvidia.com/orgs/nim/teams/nvidia/models/parakeet-1-1b-ctc-en-us_finetune).
